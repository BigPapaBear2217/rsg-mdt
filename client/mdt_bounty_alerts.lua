...
RegisterNetEvent('rsg-mdt:client:openBountyAlerts', function()
    ...
    RSGCore.Functions.TriggerCallback('rsg-mdt:server:getActiveBountyAlerts', function(alerts)
        local options = {}
        ...
        if alerts and #alerts > 0 then
            for i = 1, math.min(20, #alerts) do
                ...
                table.insert(options, {
                    ...
                    event = 'rsg-mdt:client:viewBountyAlertDetails',
                    args = {alert = alert},
                    arrow = true
                })
            end
        ...
        table.insert(options, {
            ...
            event = 'rsg-mdt:client:createBountyAlert',
            arrow = true
        })
...
-- Create Bounty Alert
------------------------------------------
RegisterNetEvent('rsg-mdt:client:createBountyAlert', function()
    ...
    if input then
        ...
        if citizenid and reason and reason ~= '' then
            ...
            RSGCore.Functions.TriggerCallback('rsg-mdt:server:createBountyAlert', function(success)
                ...
                TriggerEvent('rsg-mdt:client:openBountyAlerts')
            end, alertData)
        else
            lib.notify({
                title = 'Error',
...
------------------------------------------
RegisterNetEvent('rsg-mdt:client:viewBountyAlertDetails', function(data)
    ...
    if PlayerData.job.type == "leo" and alert.status == 'active' then
        table.insert(options, {
            ...
            event = 'rsg-mdt:client:claimBountyAlert',
    ...
    if PlayerData.job.type == "leo" then
        table.insert(options, {
            ...
            event = 'rsg-mdt:client:updateBountyAlertStatus',
            args = {alertId = alert.id},
            arrow = true
        })
    end
...
-- Claim Bounty Alert
------------------------------------------
RegisterNetEvent('rsg-mdt:client:claimBountyAlert', function(data)
    ...
    if alert == 'confirm' then
        RSGCore.Functions.TriggerCallback('rsg-mdt:server:claimBountyAlert', function(success)
            if success then
                lib.notify({
...
------------------------------------------
RegisterNetEvent('rsg-mdt:client:updateBountyAlertStatus', function(data)
    local alertId = data.alertId
    ...
    if input then
        ...
        RSGCore.Functions.TriggerCallback('rsg-mdt:server:updateBountyAlertStatus', function(success)
            if success then
                lib.notify({
...
------------------------------------------
RegisterNetEvent('rsg-mdt:client:viewPersonBountyAlerts', function(data)
    local citizenid = data.citizenid
    
    RSGCore.Functions.TriggerCallback('rsg-mdt:server:getPersonBountyAlerts', function(alerts)
        local options = {}
        ...
        table.insert(options, {
            ...
            icon = 'fa-solid fa-plus',
            event = 'rsg-mdt:client:createBountyAlertForPerson',
...
------------------------------------------
RegisterNetEvent('rsg-mdt:client:createBountyAlertForPerson', function(data)
    local citizenid = data.citizenid
