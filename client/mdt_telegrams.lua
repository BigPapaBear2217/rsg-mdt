...
-- Open Telegrams Menu
------------------------------------------
RegisterNetEvent('rsg-mdt:client:openTelegrams', function()
    -- Request telegram data from server
    RSGCore.Functions.TriggerCallback('rsg-mdt:server:getTelegrams', function(telegrams)
        local options = {}
        ...
        if telegrams and #telegrams > 0 then
            for i = 1, math.min(15, #telegrams) do
                ...
                table.insert(options, {
                    ...
                    icon = statusIcon,
                    event = 'rsg-mdt:client:viewTelegram',
...
------------------------------------------
RegisterNetEvent('rsg-mdt:client:viewTelegram', function(data)
    ...
    if telegram.status == '0' then
        table.insert(options, {
            ...
            event = 'rsg-mdt:client:markTelegramAsRead',
    ...
    table.insert(options, {
        ...
        event = 'rsg-mdt:client:replyToTelegram',
...
------------------------------------------
RegisterNetEvent('rsg-mdt:client:markTelegramAsRead', function(data)
    local telegramId = data.telegramId
    
    RSGCore.Functions.TriggerCallback('rsg-mdt:server:markTelegramAsRead', function(success)
        if success then
            lib.notify({
        ...
        TriggerEvent('rsg-mdt:client:openTelegrams')
    end, telegramId)
end)
...
-- Reply to Telegram
------------------------------------------
RegisterNetEvent('rsg-mdt:client:replyToTelegram', function(data)
    ...
    if input then
        ...
        if replySubject and replySubject ~= '' and replyMessage and replyMessage ~= '' then
            ...
            RSGCore.Functions.TriggerCallback('rsg-mdt:server:sendTelegram', function(success)
                if success then
                    lib.notify({
                        title = 'Reply Sent',
